...
from PyQt5.QtGui import QDoubleValidator, QIntValidator, QPalette, QBrush, QColor, QFont
...
from Gv3GEWRF.core import CMAQNetCDFVariable, CMAQNetCDFVariableSource
...
Dataset = namedtuple('Dataset', [
    ...
    'times', # List[str]
    'extra_dims' # Dict[str,NetCDFExtraDim]
])

class CMAQViewWidget(QWidget):
    ...
    def __init__(self, iface, dock_widget: QDockWidget) -> None:
        ...
        dir_path = '/home/ubuntu/CMAQ/CCTM/output'
        res = []
# Iterate directory
        for file in os.listdir(dir_path):
            if file.startswith('CCTM') and file.endswith('nc'):
                res.append(file)
#        print("FILES ",res)
##        self.add_dataset('/home/ubuntu/WRF-4.4/test/em_real')
        for file in res:
            cmaqout_file = '/home/ubuntu/CMAQ/CCTM/output/'+file
#            print ("File one-by-one ",wrfout_file)
            self.add_dataset(cmaqout_file)
    ...
    def create_variable_selector(self) -> None:
        ...
        self.variable_selector.header().setFont(QFont('Verdana', 12))
...
#        print ("AT create_dataset_selector")
        dataset_label = QLabel('Dataset:')
        dataset_label.setFont(QFont('Verdana', 12))
        self.dataset_selector = QComboBox()
        self.dataset_selector.currentIndexChanged.connect(self.on_dataset_selected)
        self.dataset_selector.setFont(QFont('Verdana', 12))
        hbox = QHBoxLayout()
...
#        print ("@ADD DATASET ")
        variables = Gv3GEWRF.core.get_supported_cmaq_nc_variables(path)
        print ("@ DATASET VARIABLES ",variables,"\n")
        times = Gv3GEWRF.core.get_cmaq_nc_time_steps(path)
        print ("@DATASET TIMES ",times,"\n")
        extra_dims = Gv3GEWRF.core.get_cmaq_nc_extra_dims(path)
        print ("@ADD DATASET extra_dims ",extra_dims,"\n")
        dataset_name = os.path.basename(path)
        print ("@ADD DATASET dataset_name ",dataset_name,"\n")
...
#        print ("@SELECT DATASET ")
    ...
    def replace_variable_layer(self) -> None:
        ...
        uri, dispose = Gv3GEWRF.core.convert_wrf_nc_var_to_gdal_dataset(
    ...
    def select_time_band_in_variable_layers(self) -> None:
        ...
        layers = Gv3GEWRF.plugin.geo.get_raster_layers_in_group(dataset.name)
        for layer in layers:
            var_name = layer.shortName()
            if var_name in dataset.variables:
                Gv3GEWRF.plugin.geo.switch_band(layer, time_idx)
    
    def get_variable_label(self, variable: CMAQNetCDFVariable) -> str:
    ...
    def get_variable(self) -> CMAQNetCDFVariable:
