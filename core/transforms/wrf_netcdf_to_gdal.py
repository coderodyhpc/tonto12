...
def get_supported_wrf_nc_variables(path: str) -> Dict[str,WRFNetCDFVariable]:
    ...
    try:
        ...
        for var_name in ds.variables:
            print ("COORD_VARS ",var_name,COORD_VARS)
            if var_name in COORD_VARS:
                print('Ignoring1 {}, coord var'.format(var_name))
                continue
            ...
            if len(dims) > 4:
                # should never happen
                print('Ignoring2 {}, too many dims: {}'.format(var_name, dims))
                continue

            if dims[0] != 'Time':
                # should never happen
                print('Ignoring3 {}, time dim missing, dims: {}'.format(var_name, dims))
                continue

            # TODO support staggered vars
            if dims[-2:] != ('south_north', 'west_east'):
                print('Ignoring4 {}, staggered, dims: {}'.format(var_name, dims))
                continue

            if len(dims) == 4:
                extra_dim = dims[1]
                if extra_dim not in extra_dims:
                    print('Ignoring5 {}, unsupported z dimension: {}'.format(var_name, extra_dim))
                    continue
            else:
                extra_dim = None
