...
CMAQNetCDFVariable = namedtuple('CMAQNetCDFVariable', ['name', 'description', 'units', 'extra_dim_name', 'source'])
CMAQNetCDFExtraDim = namedtuple('CMAQNetCDFExtraDim', ['name', 'label', 'steps'])

__all__ = ['CMAQNetCDFVariable', 'CMAQNetCDFExtraDim']

@export
class CMAQNetCDFVariableSource(Enum):
    FILE = auto()
...
DIAG_VARS = {
    name: CMAQNetCDFVariable(name, description, units, DIAG_DIMS[name][1] if len(DIAG_DIMS[name]) == 4 else None, CMAQNetCDFVariableSource.WRF_PYTHON)
...
@export
def convert_cmaq_nc_var_to_gdal_dataset(
    ...
    time_steps = get_cmaq_nc_time_steps(path)
...
@export
def get_supported_cmaq_nc_variables(path: str) -> Dict[str,CMAQNetCDFVariable]:
    ''' Returns all variables supported by `convert_cmaq_nc_var_to_gdal_dataset`.'''
    extra_dims = get_cmaq_nc_extra_dims(path)
    ds = nc.Dataset(path)
    try:
        variables = {}
        for var_name in ds.variables:
            print ("VARNAME ",var_name,extra_dims)
            if var_name in COORD_VARS:
                print('Ignoring-1 {}, coord var'.format(var_name))
                continue
            ...
            if len(dims) > 4:
                # should never happen
                print('Ignoring-2 {}, too many dims: {}'.format(var_name, dims))
                continue

#            if dims[0] != 'Time':
#                # should never happen
#                print('Ignoring-3 {}, time dim missing, dims: {}'.format(var_name, dims))
#                continue

#            # TODO support staggered vars
#            if dims[-2:] != ('south_north', 'west_east'):
#                print('Ignoring-4 {}, staggered, dims: {}'.format(var_name, dims))
#                continue

#            if len(dims) == 4:
#                extra_dim = dims[1]
#                if extra_dim not in extra_dims:
#                    print('Ignoring-5 {}, unsupported z dimension: {}'.format(var_name, extra_dim))
#                    continue
#            else:
#                extra_dim = None
            ...
            variables[var_name] = CMAQNetCDFVariable(
                name=var_name, description=description,
                units=units, extra_dim_name=extra_dim,
                source=CMAQNetCDFVariableSource.FILE)
...
@export
def get_cmaq_nc_extra_dims(path: str) -> Dict[str,CMAQNetCDFExtraDim]:
    ds = nc.Dataset(path)
    try:
        dims = ds.dimensions
        attrs = ds.__dict__
        extra_dims = {} # type: Dict[str,CMAQNetCDFExtraDim]
        ...
        def add_dim(name: str, label: str, step_fn: Optional[Callable[[int],Any]]=None):
            ...
            extra_dims[name] = CMAQNetCDFExtraDim(name=name, label=label, steps=steps)
