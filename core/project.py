...
from Gv3GEWRF.core.writers.namelist import patch_namelist, patch_namelist2, write_namelist
from Gv3GEWRF.core.downloaders.datasets import met_datasets_vtables

PROJECT_FILENAME = 'wrf.json'
...
#            json.dump(self.data, fp, indent=4, cls=ProjectJSONEncoder)
        with open((os.path.join('/home/ubuntu/WRF-4.4/project', PROJECT_FILENAME)), 'w') as fp:
            json.dump(self.data, fp, indent=4, cls=ProjectJSONEncoder)

    @property
    def run_wps_folder(self):
#        return os.path.join(self.path, 'run_wps')
        return ('/home/ubuntu/PREPRO/WPS')

    @property
    def run_wrf_folder(self):
#        return os.path.join(self.path, 'run_wrf')
        return ('/home/ubuntu/WRF-4.4/test/em_real')
...
#        return os.path.join(self.path, 'namelist.wps')
        return ('/home/ubuntu/PREPRO/WPS/namelist.wps')
    
    @property
    def wrf_namelist_path(self) -> str:
#        assert self.path
#        return os.path.join(self.path, 'namelist.input')
        return ('/home/ubuntu/WRF-4.4/test/em_real/namelist.input')

    @property
    def geogrid_tbl_path(self) -> str:
#        assert self.path
#        return os.path.join(self.path, 'GEOGRID.TBL')
        return ('/home/ubuntu/PREPRO/WPS/geogrid/GEOGRID.TBL')
...
#    def set_domains(self, cell_size: Tuple[float,float], domain_size: Tuple[int,int],
#                    center_lonlat: LonLat, parent_domains: List[dict]=[], map_proj: str, 
#                    truelat1: float=None, truelat2: float=None, stand_lon: float=None) -> None:
#    def set_domains(self, cell_size, domain_size,
#                    center_lonlat, parent_domains, map_proj, 
#                    truelat1, truelat2, stand_lon) -> None:
    def set_domains(self, map_proj: str=[],
                    cell_size: Tuple[float,float]=[None,None], domain_size: Tuple[int,int]=[None,None],
                    center_lonlat: LonLat=[None,None],
...
#        print ("I'M AT SET_DOMAINS MAP_PROJ ",map_proj," CENTER_LONLAT ",center_lonlat)
#        print ("I'M AT SET_DOMAINS MAP_PROJ2 ",cell_size[0], cell_size[1], domain_size[0], domain_size[1])
        self.data['domains'] = [{
            "map_proj": map_proj,
            "center_lonlat" : [center_lonlat.lon, center_lonlat.lat],
            "cell_size": [cell_size[0], cell_size[1]],
...
#        print ("MAP_PROJ ",main_domain['truelat1'],main_domain['truelat2'],main_domain['stand_lon'])
...
#        print ("At fill domains ",domains[0])
#        print ("\nAt fill domains@project.py ",domains[0],'\n')
    ...
    def update_wps_namelist(self, puer):
        ...
        self.puer = puer
        self.fill_domains()
        wps = convert_project_to_wps_namelist(self, self.puer)
    ...
    def update_wrf_namelist(self, puer):
        from Gv3GEWRF.core.transforms.project_to_wrf_namelist import convert_project_to_wrf_namelist
        print ("At update_wrf_namelist ")
#        self.fill_domains()
        self.puer = puer
        nml_patch = convert_project_to_wrf_namelist(self, self.puer)
        ...
        patch_namelist2(self.wrf_namelist_path, nml_patch)

    # TODO move prepare functions into separate module together with functions for running
    def prepare_wps_run(self, wps_folder: str, servus) -> None:
        self.servus = servus
...
#        os.makedirs(self.run_wps_folder, exist_ok=True)
        ...
        self.update_wps_namelist(self.servus)
...
# NOT SURE WHY IT"S TRYING TO COPY GEOGRID>TBL

        metgrid_folder = os.path.join(self.run_wps_folder, 'metgrid')
        print (metgrid_folder)
...
# NOT SURE WHY IT"S TRYING TO COPY METGRID.TBL

#        shutil.copy(self.wps_namelist_path, self.run_wps_folder)
# HERE IT"S TRYING TO COPY namelist.wps
...
#___ Preparation of namelist.input                    
    def prepare_wrf_run(self, wrf_folder: str, servus) -> None:
        self.servus = servus
#        print ("@ prepare_wrf_run ",wrf_folder,self.servus.satus_dies)
#        if not os.path.exists(wrf_folder):
#            raise WRFDistributionError(f'{wrf_folder} does not exist')
        self.update_wrf_namelist(self.servus)
        os.makedirs(self.run_wrf_folder, exist_ok=True)

#!!! If I'm running from the directory (instead of from a project subdirectory, I need to keep all the original files)             
...
#!!! This might be useful when I have a project rather than running from the directory            
